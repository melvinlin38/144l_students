...
#Import Data
```{r}
count.tab <- read_rds("~/GITHUB/144l_students/Input_Data/week6/seqtab-nochimtaxa.rds") #table of counts for each sequence in each sample
tax.tab <- read_rds("~/GITHUB/144l_students/Input_Data/week6/taxa.rds") #table that matches ASV to sequence
sample.tab <- read_rds("~/GITHUB/144l_students/Output_Data/week4/ACIDD_Exp_Processed_DOC_BGE.rds") %>% 
...
mean(sample_sums(sub_ps)) #7695
mean(sample_sums(ps_min)) #6047
...
set.seed(1)
# Ordinate
nmds <- ordinate(ps_min, method = "NMDS", distance = "bray")
#stress = 0.04
```
